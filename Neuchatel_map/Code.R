...
city_coords <- getbb("Neuchatel Switzerland")
...
labels <- as.data.frame(matrix(nrow=1,ncol=3))
colnames(labels) <- c("X","Y","Name")
labels[1,1] <- 6.982346
labels[1,2] <- 46.995077
labels[1,3] <- "Neuchatel Lac"
...
ggplot() +
  ...
  geom_sf(data = water$osm_multipolygons,
          ...
          size = .2,
          alpha = .5) +
  geom_text(data = labels, aes(X, Y, label = Name), colour = "white") +
  coord_sf(xlim = city_coords[1,],
           ylim = city_coords[2,],
           expand = TRUE) +
  theme_void() +
  labs(title = "Neuchâtel",
...
ggsave("Cities map/Neuchatel_map/plot.png", width = 8, height = 8)
